...
static const int lol_signals[LOL_NUM_SIGHANDLERS + 1] = {
                 SIGHUP, SIGUSR1, SIGINT, SIGTERM, SIGSEGV, 0
};

/* ********************************************************** */
void lol_memset_indexbuffer(const alloc_entry value, const alloc_entry num) {
  alloc_entry i = 0;
  for (; i < num; i++) {
...
void lol_restore_sighandlers() {
     ...
     for (i = 0; i < LOL_NUM_SIGHANDLERS; i++) {
          sigaction(lol_signals[i],  &lol_sighand_store[i],  NULL);
     }
...
/* ********************************************************** */
static void lol_freemem_hdl(int sig, siginfo_t *siginfo, void *context)
{
 ...
 *  0 : success
 * ********************************************************** */
int lol_remove_nentry(FILE *f, const DWORD num_blocks, const DWORD nentry, int remove_idx) {
 ...
 *  0 : success
 * ********************************************************** */
int lol_set_index_value(FILE *f, const DWORD num_blocks, const alloc_entry idx, const alloc_entry new_value) {
...
int lol_index_malloc(const size_t num_entries) {
  ...
  if (num_entries > LOL_STORAGE_SIZE && lol_index_buffer) {
...
/* ********************************************************** */
size_t lol_fill_with_value(const alloc_entry value, size_t bytes, FILE *s) {
...
BOOL lol_is_validfile(char *name) {
  ...
  struct lol_super sb;
   ...
   if (!name)
       return 0;
   if ((lol_get_vdisksize(name, &sb, NULL, RECUIRE_SB_INFO) < LOL_THEOR_MIN_DISKSIZE))
	return 0;
	...
	if (fread((char *)&current_index, (size_t)(ENTRY_SIZE), 1, vdisk) != 1) {
	  ...
	  if (terminate) {
		 ...
		 current_index = LAST_LOL_INDEX;
	         if (fwrite((char *)&current_index, (size_t)(ENTRY_SIZE), 1, vdisk) != 1) {
                      goto error;
                 }
		 ret = 0;
		 goto error;
...
int lol_supermod(FILE *vdisk, struct lol_super *sb, const int func) {
  ...
  if (fwrite((const char *)&s, (size_t)(DISK_HEADER_SIZE), 1, vdisk) != 1)
       goto error;
...
/* **********************************************************
 * lol_add_file_to_super: PRIVATE FUNCTION!
 * Increment the superblock file counter.
 * Does not do anything else. op->sb must be set
 * and the file must be open for writing alreafdy.
 * ********************************************************** */
int lol_add_file_to_super(lol_FILE *op) {
    return lol_supermod(op->vdisk, &(op->sb), LOL_INCREASE);
}
/* **********************************************************
 * delete_chain_from: PRIVATE FUNCTION!
 * Delete fat chain of the argument file.
...
int lol_delete_chain_from(lol_FILE *op, const int flags) {
  ...
  if (!vdisk || !num_blocks || !block_size || first_index > last_index)
    return -2;
  ...
  if (flags)
    ...
    while(1) {
      ...
      fread((char *)&current_index, (size_t)(ENTRY_SIZE), 1, vdisk);

      if (current_index > last_index) {
	return -3;
      }
        // Clear and read next entry
        fseek(vdisk, -(ENTRY_SIZE), SEEK_CUR);

	if (j) {
                 fwrite((const char *)&empty, (size_t)(ENTRY_SIZE), 1, vdisk);
	}
        else {
               fwrite((const char *)&last, (size_t)(ENTRY_SIZE), 1, vdisk);
               j = 1;
	}
...
/* ********************************************************** *
 * lol_get_free_index:
 * Find the first free index in fat and mark it as used.
 * Return:
 * < 0 : error
 *   0 : success
 * ********************************************************** */
...
/* ********************************************************** */
int verify_lol_disk_size(lol_FILE *op) {
  DWORD size;
...
/* ********************************************************** */
int lol_write_modified_nentry(lol_FILE *op) {
  ...
  if (LOL_GOTO_DENTRY(op))
    ...
    if (fread((char *)&entry, (size_t)(NAME_ENTRY_SIZE), 1, op->vdisk) != 1) { // Read the timestamp to entry.
	goto error;
    }
	...
	if (fseek(op->vdisk, -(NAME_ENTRY_SIZE), SEEK_CUR)) {
        ...
        entry.created   = time(NULL); // We don't have "modified" field
 ...
 error:
  fsetpos(op->vdisk, &pos);
  return -7;
} // end lol_write_modified_nentry
/* ********************************************************** */
int lol_write_modified_chain(lol_FILE *op, const long olds, const long news, const alloc_entry last_old) {
...
int lol_read_index_chain(lol_FILE *op, const size_t read_blocks)
{
  ...
  if (first_index > last_index || first_index < 0) {
   ...
   // skip = DISK_HEADER_SIZE + num_blocks * NAME_ENTRY_SIZE;
 ...
 *
 * ********************************************************** */
int lol_create_index_chain(lol_FILE *op, const long olds, const long news, alloc_entry *last_old)
{
...
size_t lol_fread(void *ptr, size_t size, size_t nmemb, lol_FILE *op)
{
  ...
  if (!op->vdisk || op->opened != 1) {
       op->err = lol_errno = EBADFD; return 0;
  }
  ...
  if (end_bytes) {
	 ...
	 if (current_index < 0) {
        ...
        amount = lol_rw_data_block(op, current_index, ptr, end_bytes, LOL_READ);
	if (amount < 0) {
	                  off = 0;
			  goto error;
        }

	off += amount; // was: += end_bytes
  ...
  // Process the full blocks
          ...
          for (; i < block_loops; i++) {
			...
			if (current_index < 0) {
			  ...
			  //printf("ERROR block: corrupted index %d\n", (int)current_index);
			  op->err = lol_errno = EFAULT;
			  goto error;
                          
			}

		 amount = lol_rw_data_block(op, current_index, ptr+off, block_size, LOL_READ);

                 if (amount < 0) {
		   // printf("lol_fread: BLOCK: ERROR lol_rw_data_block %d\n", (int)amount);
	                  off = 0;
			  goto error;
                  }

		 off += amount; // was: += block_size
           ...
           if (start_bytes) {
			...
			if (current_index < 0) {
			  // This should not happen, unless corrupted file
			  // printf("lol_fread: START ERROR start: corrupted index %d\n", (int)current_index);
			  op->err = lol_errno = EFAULT;
			  goto error;
                          
			}

	                amount = lol_rw_data_block(op, current_index, ptr + off, start_bytes, LOL_READ);

                        if (amount < 0) {
			  // printf("lol_fread: start_bytes: ERROR lol_rw_data_block %d\n", (int)amount);

	                                  off = 0;
			                  goto error;
                        }

			off += amount; // was: += start_bytes
...
size_t lol_fwrite(const void *ptr, size_t size, size_t nmemb, lol_FILE *op)
{
  ...
  if (verify_lol_disk_size(op)) { // Has someone tampered with our fd?
      op->err = lol_errno = EBADFD; return 0;
  }
  ...
  write_blocks = lol_compute_new_indexes(op, (long)amount, &olds, &mids, &news, &new_filesize);
  ...
  if (end_bytes) {
      ...
      current_index = lol_index_buffer[i++];
			if (current_index < 0) {
			  //printf("SHOULD NOT HAPPEN: lol_fwrite start_bytes: current_index < 0\n");
			  op->err = lol_errno = EFAULT;
			  goto error;
                          
			}

      amount = lol_rw_data_block(op, current_index, (char *)ptr, end_bytes, LOL_WRITE);
  ...
  // Process the full blocks
		 ...
		 amount = lol_rw_data_block(op, current_index, (char *)(ptr+off), block_size, LOL_WRITE);
    ...
    if (start_bytes) {
	  ...
	  amount = lol_rw_data_block(op, current_index, (char *)ptr + off, start_bytes, LOL_WRITE);
               if (amount < 0) {
	                     off = 0;
			     goto error;
               }

               off += amount; // start_bytes;
  ...
  ret = 0;
     ...
     ret = lol_write_modified_nentry(op);

     if (ret)
         op->err = lol_errno = EIO;

  if (new_filesize != filesize) {
       ret = lol_write_modified_chain(op, olds, news, last_old);
...
int lol_unlink(const char *name) {
  ...
  if (lol_delete_chain_from(fp, LOL_UNLINK))
     goto error;
...
int lol_stat(const char *path, struct stat *st) {
  ...
  if (raw_size <  LOL_THEOR_MIN_DISKSIZE) {
      ...
      for (i = 0; i < num_blocks; i++) {
        ...
        st->st_blocks = (blkcnt_t)(st->st_size / 512); // FAKE!
        if (st->st_size % 512)
	  st->st_blocks++;
