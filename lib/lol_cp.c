...
int copy_from_disk_to_lolfile(int argc, char *argv[]) {
  ...
  if (stat (vdisk, &sta)) {
	printf(lol_cantread_txt, me, vdisk);
        return -1;
  }
  ...
  for (i = 1; i < num_files; i++) {
    ...
    if (stat (argv[i], &st)) {
	printf(lol_cantread_txt, me, argv[i]);
        continue;
    }
    ...
    if (dst_size < 0) {
	printf(lol_cantuse_txt, me, vdisk);
        return -1;
    }
#if LOL_TESTING
    ...
    if (!(lol_stat(name, &sta))) {
      // Exixting file !
       printf("lol %s: \'%s\' exists. Replace [y/n]? ", me, name);
       answer = (char)getchar();
       (void)getchar();
       if (answer != 'y')
	 continue;
...
#if LOL_TESTING
	printf("DEBUG: lol cp: lol_fopen failed. lol_errno = %d\n", lol_errno);
#endif
          continue;
    }
